				  ...
				  char *resolved, size_t resolved_size)
{
	...
	if (WSRCONFIG(ws)->n_group_list == 0 && ws->cert_groups_size == 0) {
	...
	for (i = 0; i < ws->cert_groups_size; i++) {
		if (strcmp(ws->cert_groups[i], group) == 0) {
			strlcpy(resolved, ws->cert_groups[i], resolved_size);
			return 1;
		}
	}
...
static void set_auth_groupname(worker_st *ws, SecAuthInitMsg *ireq,
	...
	if (ws->auth_state == S_AUTH_INACTIVE) {
		...
		if (ret < 0) {
			...
			if (WSRCONFIG(ws)->default_select_group != NULL &&
				...
				set_auth_groupname(ws, &ireq, groupname);
			} else {
		...
		if (ws->selected_auth->type & AUTH_TYPE_CERTIFICATE) {
			...
			if (ireq.group_name == NULL &&
			    unlisted_groupname[0] != 0) {
				/* Certificate auth checks the selected group before auth_type is set. */
				set_auth_groupname(ws, &ireq,
						   unlisted_groupname);
			}
		...
		if (ireq.group_name == NULL && unlisted_groupname[0] != 0 &&
		    ireq.auth_type != 0) {
			set_auth_groupname(ws, &ireq, unlisted_groupname);
		}
