			     ...
			     unsigned int len)
{
	...
	if (pctx->groupnames_size >= MAX_GROUPS) {
		...
		while (p2 != NULL) {
			...
			oc_syslog(LOG_DEBUG, "radius-auth: found group %s", gname);
	...
	if (ret == OK_RC) {
		...
		while (vp != NULL) {
			if (vp->attribute == PW_SERVICE_TYPE &&
				...
				/* Group-Name */
				parse_groupnames(pctx, vp->strvalue, vp->lvalue);
...
int radius_auth_helper_main(int fd)
{
	...
	nas_identifier = read_counted_string(pool, fd,
					     sizeof(vctx->nas_identifier) - 1);
	group_separator = read_counted_string(pool, fd,
					      sizeof(vctx->group_separator) - 1);
	if (config == NULL || nas_identifier == NULL || group_separator == NULL)
		return EXIT_FAILURE;

	strlcpy(vctx->config, config, sizeof(vctx->config));
	strlcpy(vctx->nas_identifier, nas_identifier, sizeof(vctx->nas_identifier));
	if (group_separator[0] != 0)
		strlcpy(vctx->group_separator, group_separator,
			sizeof(vctx->group_separator));
	else
		strlcpy(vctx->group_separator, ";", sizeof(vctx->group_separator));
...
int radius_auth_pass_async_apply(void *ctx, int fd, int *auth_ret)
{
	...
	if (hdr.routes_size > 0) {
		pctx->routes =
			talloc_zero_size(pctx, sizeof(char *) * hdr.routes_size);
		if (pctx->routes == NULL)
			return -1;
	}
